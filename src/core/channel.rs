...
/// A channel that can be used to draw random elements from a [Sha256Hash] digest.
...
impl Channel for Sha256Channel {
    ...
    fn mix_digest(&mut self, digest: Self::Digest) {
        self.digest = Sha256Hasher::concat_and_hash(&self.digest, &digest);
        self.channel_time.inc_challenges();
    }
    ...
    fn mix_nonce(&mut self, nonce: u64) {
        ...
        self.digest =
            Sha256Hasher::concat_and_hash(&self.digest, &Sha256Hash::from(padded_nonce));
        self.channel_time.inc_challenges();
    }
...
mod tests {
    ...
    #[test]
    fn test_initialize_channel_sha256() {
        let initial_digest = Sha256Hash::from(vec![0; 32]);
        let channel = Sha256Channel::new(initial_digest);
    ...
    fn test_channel_time_sha256() {
        ...
        channel.mix_digest(Sha256Hash::from(vec![1; 32]));
        assert_eq!(channel.channel_time.n_challenges, 1);
        assert_eq!(channel.channel_time.n_sent, 0);
    ...
    #[test]
    fn test_draw_random_bytes_sha256() {
        let initial_digest = Sha256Hash::from(vec![1; 32]);
        let mut channel = Sha256Channel::new(initial_digest);
    ...
    #[test]
    pub fn test_draw_felt_sha256() {
        let initial_digest = Sha256Hash::from(vec![2; 32]);
        let mut channel = Sha256Channel::new(initial_digest);
